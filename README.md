...
- Open `SPC_model.slx`
  ...
  - Run the model
  - To view the results open the `Scope` blocks showing the relevant variables.
...
These values go into the `motor_PWMController`.
There the acceleration value drives a PWM-signal which drives the H-bridge connected to a DC-Motor. The deceleration signal toggles the breaking functionality on the H-bridge, which enable regenerative breaking on the motors. We have not implemented an other breaking system, since this could be handled by the car pulling the caravan. Additionally, regenerative breaking if sufficient most of the time.
...
| rollingResistCoeff | The rolling resistance coefficient is determined by the friction in the gearing and in the axle of the wheel. As well as the friction between the wheel and the road.                                                                                                                                                                                                                                                                 |
...
![SPC_model_image](images/SPC_model.png)

### Motor PWM Controller

![Motor_image](images/motor_PWMcontroller.png)

### Mechanical model (Point mass)

![Mechanical_image](images/mechanicalbody_model.png)

### Battery model

![Battery_image](images/battery_model.png)

### Range estimation and efficiency calculation

![Range_image](images/ranging.png)
...
- Simulate the airodynamics of pulling a caravan
  ...
  - Requires CFD
- Battery charge ballancing between the caravan and the pulling car.
